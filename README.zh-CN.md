...
## 1.2.0 版本：多 Provider / 多模型 AI

- DeepSeek 仍然是默认服务，并兼容 1.1.x 的旧设置。
- 新增 Google Gemini（使用 Google 的 OpenAI-compatible 接口）。
- 新增 OpenAI Chat Completions。
- 新增自定义 OpenAI-compatible 服务；需要时 Chrome 会请求对应 HTTPS API 域名的访问权限。
- 翻译、解释、概览/分析、笔记润色可以分别指定默认模型。
- Settings 新增连接测试按钮。
- API 密钥仍保存在 Chrome 扩展本地存储中；字幕内容只发送给该任务实际选择的 AI 服务。
